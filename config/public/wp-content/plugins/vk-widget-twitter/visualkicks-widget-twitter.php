...
/*
Plugin Name: Visualkicks - Twitter Widget
Plugin URI: http://www.visualkicks.com/plugins
Description: Enables a Twitter widget for use in our compatible Visualkicks themes 
Version: 1.1
Author: Visualkicks
...
12/11/2014 - Version 1.1
- removed twitter stylesheet. Styling should be handled by themes.


/*-----------------------------------------------------------------------------------*/

	// widget init
	add_action( 'widgets_init', 'vk_twitter_widget' );

	// register widget
	function vk_twitter_widget() {

		register_widget( 'VK_Twitter_Widget' );
...
/*-----------------------------------------------------------------------------------*/
	...
	function VK_Twitter_Widget() {
		...
		$control_ops = array( 
		    'width' => 300, 
		    'height' => 350,
		    'id_base' => 'vk_twitter_widget'
		);

		// create
		$this->WP_Widget( 'vk_twitter_widget', __('Twitter Feed', 'framework'), $widget_ops, $control_ops );
		...
		if ( $title ) { echo $before_title . $title . $after_title; }
			...
			if($diff >= $crt || empty($tp_twitter_plugin_last_cache_time)){
				...
				function getConnectionWithAccessToken($cons_key, $cons_secret, $oauth_token, $oauth_token_secret) {
				  $connection = new TwitterzOAuth($cons_key, $cons_secret, $oauth_token, $oauth_token_secret);
				  return $connection;
				}
			...
			if(!empty($tp_twitter_plugin_tweets)){
				...
				<div class="twitter">
					...
					foreach($tp_twitter_plugin_tweets as $tweet){								
						print '<li><span>'.convert_links($tweet['text']).'</span><br /><a class="time" target="_blank" href="http://twitter.com/'.$instance['username'].'/statuses/'.$tweet['status_id'].'">'.relative_time($tweet['created_at']).'</a></li>';
...
/*-----------------------------------------------------------------------------------*/
	...
	function form($instance) {
		...
		echo '
		<p>Get your twitter keys by creating a twitter <a href="https://dev.twitter.com/apps/new">app</a></p>
		<p><label>Title:</label>
